...
namespace NLTGLO.Model
{
    public static class ExcelHelper
    {
        ...
        public static void FillAuxialryIntoContext(FileInfo file, Context toFill)
        {
            toFill.ExpectedWorkload = new Dictionary<string, int>();
            using (ExcelPackage xlPackage = new ExcelPackage(file))
            {
                Console.WriteLine("Auxilary File loaded");

                ExcelWorksheet sheet = xlPackage.Workbook.Worksheets[0]; /// this workbook has only one sheet, with header row

                Dictionary<string, bool> checklist = new Dictionary<string, bool>();
                foreach(Instructor instructor in toFill.Instructors)
                {
                    checklist[instructor.Name] = false; /// so we can check if we have read everyone from the auxilary input
                }

                int row = 2; /// we have a header row
                ...
                while (sheet.Cells[row, 1].Text != "") ///
                {
                    ...
                    {
                        Console.WriteLine("Unknown key from auxilary input: " + name+"\nExiting...");
                        return;
                    }
                    checklist[name] = true;

                    toFill.ExpectedWorkload[name] = Int32.Parse(sheet.Cells[row, 2].Text);

                    row++;
                }


                foreach (string check in checklist.Keys)
                {
                    if(!checklist[check])
                    {
                        Console.WriteLine("Unchecked teacher: " + check);
                        return;
                    }
                }
            }
        ...
        public static Context Read(FileInfo existingFile)
        {
            ...
            using (ExcelPackage xlPackage = new ExcelPackage(existingFile))
            {
                ...
                ExcelWorksheet ws_students = xlPackage.Workbook.Worksheets[0];
